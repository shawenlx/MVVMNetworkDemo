...
//        
//        // 3.显示日期
//        self.lastUpdatedTimeLabel.text = [NSString stringWithFormat:@"Last update：%@", time];//[NSString stringWithFormat:@"最后更新：%@", time];
//    } else {
//        self.lastUpdatedTimeLabel.text = @"Last update：No record";//@"最后更新：无记录";//
//    }
    ...
    if (lastUpdatedTime) {
        ...
        NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
        if ([cmp1 day] == [cmp2 day]) { // 今天
             formatter.dateFormat = @"HH:mm";//@"今天 HH:mm";
        } else if ([cmp1 year] == [cmp2 year]) { // 今年
            formatter.dateFormat = @"MM-dd HH:mm";
        } else {
            formatter.dateFormat = @"yyyy-MM-dd HH:mm";
